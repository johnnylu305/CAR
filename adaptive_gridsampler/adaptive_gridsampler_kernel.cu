...
template <typename scalar_t>
__global__ void kernel_adaptive_gridsampler_back(
const torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> img,
const torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> kernels,
const torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> offsets_h,
const torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> offsets_v,
...
const size_t n)
{
    ...
    for(int k_y = 0; k_y < k_size; ++k_y)
    {
        for(int k_x = 0; k_x < k_size; ++k_x)
        {
            ...
            scalar_t h_val = 0;
            h_val += kernels[idb][k_size * k_y + k_x][idy][idx] * (1 - beta) * (img[idb][idc][yT][xR]-img[idb][idc][yT][xL]);
			//*k
            h_val +=  kernels[idb][k_size * k_y + k_x][idy][idx] * beta * (img[idb][idc][yB][xR]-img[idb][idc][yB][xL]);

            scalar_t v_val = 0;
            v_val += kernels[idb][k_size * k_y + k_x][idy][idx] * (1 - alpha) * (img[idb][idc][yB][xL]-img[idb][idc][yT][xL]);
			//*k
            v_val +=  kernels[idb][k_size * k_y + k_x][idy][idx] * alpha * (img[idb][idc][yB][xR]-img[idb][idc][yT][xR]);

			H[idb][k_size * k_y + k_x][idy][idx] += (h_val*output[idb][idc][idy][idx]);
			V[idb][k_size * k_y + k_x][idy][idx] += (v_val*output[idb][idc][idy][idx]);
            K[idb][k_size * k_y + k_x][idy][idx] += (val*output[idb][idc][idy][idx]);
        }
    }
}

void adaptive_gridsampler_kernel_backward(const torch::Tensor& img, const torch::Tensor& kernels, const torch::Tensor& offsets_h, const torch::Tensor& offsets_v, const int offset_unit, const int padding, torch::Tensor& output, torch::Tensor& K, torch::Tensor& H, torch::Tensor& V)
{
    kernel_adaptive_gridsampler_back<float><<<(output.numel() + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE>>>(
    img.packed_accessor32<float, 4, torch::RestrictPtrTraits>(), kernels.packed_accessor32<float, 4, torch::RestrictPtrTraits>(),
    offsets_h.packed_accessor32<float, 4, torch::RestrictPtrTraits>(), offsets_v.packed_accessor32<float, 4, torch::RestrictPtrTraits>(), offset_unit, padding,
    ...
    V.packed_accessor32<float, 4, torch::RestrictPtrTraits>(), output.numel());
